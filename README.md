...
## 👨‍💻 项目信息
- **技术栈**: Java Spring Boot + 阿里云 Qwen API
- **当前版本**: v1.1.0
...
- 🎨 现代化 UI 界面，渐变色彩设计
- 🤖 **多模型支持**: Qwen Plus/Max/Turbo、DeepSeek V3 自由切换
- 💬 **多轮对话**: 支持上下文记忆，实现连续对话
- 🌡️ 可调节的 Temperature 参数，控制 AI 创造性
- ⌨️ 智能输入框，支持快捷键（Enter 发送/Shift+Enter 换行）
- 🗑️ 一键清除聊天记录
- 🔒 API Key 安全配置（环境变量）
- ⚡ 快速响应，流畅体验
...
**A**: 日常对话推荐 0.5，需要严谨回答时调低到 0.3，创意写作可调高到 0.9

### Q4: 如何修改默认端口 8080？
**A**: 在 `application.properties` 中添加 `server.port=8081或者你想要的端口`

### Q5: 支持哪些模型？
**A**: 目前支持 Qwen Plus（平衡）、Qwen Max（最强）、Qwen Turbo（最快）、DeepSeek V3 四种模型，可在页面顶部切换

### Q6: 如何清除聊天记录？
**A**: 点击底部左侧的 "🗑️ 清除记录" 按钮即可清空所有对话历史
...
### v1.1.0 (当前版本)
- ✨ **新增多模型支持**: 支持 Qwen Plus、Qwen Max、Qwen Turbo、DeepSeek V3 四种模型切换
- ✨ **新增多轮对话**: 支持对话历史记忆,实现真正的上下文连续对话
- ✨ **新增清除记录功能**: 一键清空所有聊天记录(带确认提示)
- ✨ **智能输入框**: 自动调整高度(50px-150px),支持 Enter 发送、Shift+Enter 换行
- 🎨 **UI 全面优化**: 
  - 模型选择器移至顶部,更加直观
  - 消息气泡左右分布(用户右/AI 左)
  - 温度滑块默认值调整为 0.5
